...
void main() {
  final checkoutFile = File('e:/Enything/supabase/migrations/20260896000000_100x_unauthenticated_ghost_order_ddos.sql');
  final getNearbyShopsCatsFile = File('e:/Enything/supabase/migrations/20260850000000_get_nearby_shops_categories.sql');
  final geospatialLimitsFile = File('e:/Enything/supabase/migrations/20260820000000_geospatial_limit_caps.sql');
  final trendingFile = File('e:/Enything/supabase/migrations/20271124000001_get_trending_keywords_geospatial.sql');
  ...
  outSql.writeln('-- =============================================================================');
  outSql.writeln('-- Migration: 20271124000002_100x_geospatial_admin_radius_fortress.sql');
  outSql.writeln('-- Description: Enforces the admin-defined `max_delivery_radius_km` limit');
  outSql.writeln('--              from `platform_config` across all geospatial and checkout RPCs.');
  outSql.writeln('--              Prevents users from bypassing radius limits via distance spoofing.');
  outSql.writeln('-- =============================================================================');
  outSql.writeln('');
...
''';
  ...
  // 1. get_nearby_shops
  final nearbyMatch = RegExp(r'CREATE OR REPLACE FUNCTION public\.get_nearby_shops.*?BEGIN', dotAll: true).firstMatch(nearbyShopsSql);
  if (nearbyMatch != null) {
    ...
    final body = rest.substring(0, endIndex + r'$$ LANGUAGE plpgsql SECURITY DEFINER;'.length);
  ...
  final searchShopsMatch = RegExp(r'CREATE OR REPLACE FUNCTION public\.search_shops_geospatial.*?BEGIN', dotAll: true).firstMatch(geospatialSql);
  ...
  final searchProductsMatch = RegExp(r'CREATE OR REPLACE FUNCTION public\.search_products_geospatial.*?BEGIN', dotAll: true).firstMatch(geospatialSql);
  ...
  final trendingMatch = RegExp(r'CREATE OR REPLACE FUNCTION public\.get_trending_keywords_geospatial.*?\$\$', dotAll: true).firstMatch(trendingSql);
  if (trendingMatch != null) {
    String funcDef = trendingMatch.group(0)!;
    funcDef = funcDef.replaceFirst('LANGUAGE sql', 'LANGUAGE plpgsql').replaceFirst('STABLE', '');
  ...
  final placeOrdersMatch = RegExp(r'CREATE OR REPLACE FUNCTION public\.place_orders_transaction.*?SECURITY DEFINER AS \$\$', dotAll: true).firstMatch(checkoutSql);
  if (placeOrdersMatch != null) {
    String funcDef = placeOrdersMatch.group(0)!;
    final lastDollarDollar = checkoutSql.lastIndexOf(r'$$');
    final body = checkoutSql.substring(placeOrdersMatch.end, lastDollarDollar + 2);
  ...
  v_global_max_radius := GREATEST(COALESCE(v_global_max_radius, 15.0), 0.0);'''
      ...
      final finalBody = modifiedBody.replaceAll(
      ...
      final veryFinalBody = finalBody.replaceFirst('v_order jsonb;', 'v_global_max_radius numeric;\\n  v_order jsonb;');
  ...
  File('e:/Enything/supabase/migrations/20271124000002_100x_geospatial_admin_radius_fortress.sql').writeAsStringSync(outSql.toString());
