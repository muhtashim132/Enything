...
class _RiderInsightsPageState extends State<RiderInsightsPage> {
  ...
  Map<int, Map<int, int>> _peakHours = {}; // DayOfWeek (1-7) -> Hour (0-23) -> Count
    ...
    try {
      ...
      for (var o in orders) {
        ...
        if (o['status'] == 'delivered') {
          ...
          if (o['arrived_at_shop_time'] != null && o['order_ready_time'] != null) {
      ...
      _topEarningsDays = sortedEarnings.take(5).map((e) => {
        'date': e.key,
        'amount': e.value
      }).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Rider Insights', style: GoogleFonts.outfit(fontWeight: FontWeight.w700)),
        centerTitle: true,
  ...
  Widget _statCard(String title, String val, IconData icon, Color color, bool isDark) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(title, style: GoogleFonts.outfit(color: isDark ? Colors.white70 : Colors.black54, fontSize: 13)),
          Text(val, style: GoogleFonts.outfit(color: isDark ? Colors.white : Colors.black, fontSize: 24, fontWeight: FontWeight.w800)),
        ],
      ),
    );
  }
  ...
  Widget _buildRatingCard(bool isDark) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Customer Ratings', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black)),
          const SizedBox(height: 20),
          Row(
            children: [
              Column(
                children: [
                  Text(_averageRating.toStringAsFixed(1), style: GoogleFonts.outfit(fontSize: 42, fontWeight: FontWeight.w900, color: AppColors.primary)),
                  Row(
                ...
                child: Column(
                  children: [5,4,3,2,1].map((stars) {
                    ...
                    return Padding(
                      ...
                      child: Row(
                        children: [
                          Text('$stars', style: GoogleFonts.outfit(color: isDark ? Colors.white70 : Colors.black54, fontSize: 12)),
  ...
  Widget _buildPeakHoursCard(bool isDark) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Peak Activity Hours', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black)),
          const SizedBox(height: 8),
          Text('Based on your accepted orders', style: GoogleFonts.outfit(fontSize: 13, color: isDark ? Colors.white54 : Colors.black54)),
          const SizedBox(height: 20),
          SizedBox(
            height: 140,
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: count > 0 ? AppColors.accent : (isDark ? Colors.white10 : Colors.grey.shade200),
                          borderRadius: BorderRadius.circular(4),
                        ),
                      ),
                      const SizedBox(height: 8),
                      Text(hour % 4 == 0 ? '$hour' : '', style: GoogleFonts.outfit(fontSize: 10, color: isDark ? Colors.white54 : Colors.black54)),
                    ],
                  ),
                );
              },
  ...
  Widget _buildTopEarningsCard(bool isDark) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Top Earning Days', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black)),
          const SizedBox(height: 16),
          if (_topEarningsDays.isEmpty)
             Text('No data available yet.', style: GoogleFonts.outfit(color: isDark ? Colors.white54 : Colors.black54)),
            ...
            return Padding(
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    radius: 12,
                    backgroundColor: idx == 0 ? Colors.amber : (isDark ? Colors.white10 : Colors.grey.shade200),
                    child: Text('${idx + 1}', style: GoogleFonts.outfit(fontSize: 12, color: idx == 0 ? Colors.white : (isDark ? Colors.white54 : Colors.black54), fontWeight: FontWeight.bold)),
                  ),
                  const SizedBox(width: 12),
                  Text(date, style: GoogleFonts.outfit(fontSize: 15, fontWeight: FontWeight.w600, color: isDark ? Colors.white : Colors.black87)),
                  const Spacer(),
                  Text('₹${(amt as double).toStringAsFixed(0)}', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.success)),
                ],
              ),
            );
          }),
