...
class _DeliveryKycUploadPageState extends State<DeliveryKycUploadPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // Upload Images
      final aadharFrontUrl = _aadharFront != null
          ? await _uploadFile(_aadharFront!, '${userId}_aadhar_front')
          : null;
      final aadharBackUrl = _aadharBack != null
          ? await _uploadFile(_aadharBack!, '${userId}_aadhar_back')
          : null;
      final panFrontUrl = _panFront != null
          ? await _uploadFile(_panFront!, '${userId}_pan_front')
          : null;
      final panBackUrl = _panBack != null
          ? await _uploadFile(_panBack!, '${userId}_pan_back')
          : null;
      final dlFrontUrl = _dlFront != null
          ? await _uploadFile(_dlFront!, '${userId}_dl_front')
          : null;
      final dlBackUrl = _dlBack != null
          ? await _uploadFile(_dlBack!, '${userId}_dl_back')
          : null;
      final rcFrontUrl = _rcFront != null
          ? await _uploadFile(_rcFront!, '${userId}_rc_front')
          : null;
      final rcBackUrl = _rcBack != null
          ? await _uploadFile(_rcBack!, '${userId}_rc_back')
          : null;
      ...
      if (mounted) {
        ...
        for (final admin in (adminRows as List)) {
          notifProvider.sendBackgroundPush(
            ...
            title: '🛵 New Rider KYC!',
            body:
                'A delivery partner has submitted their KYC documents for review.',
          );
        }
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.security_rounded,
                          color: Color(0xFF51CF66), size: 32),
                ...
                _buildInputField('Aadhaar Number *', _aadharCtrl,
                    isNumber: true, hint: '12-digit Aadhaar Number'),
                _buildInputField('PAN Number *', _panCtrl,
                    isCaps: true, hint: 'ABCDE1234F'),
                _buildInputField('Driving License Number *', _dlCtrl,
                    isCaps: true, hint: 'DL-1420110012345'),
                const SizedBox(height: 32),
                _buildSectionTitle('Bank Account Details'),
                _buildInputField('Account Holder Name *', _accountHolderCtrl,
                    hint: 'Name exactly as per bank'),
                _buildInputField('Account Number *', _bankAccountCtrl,
                    isNumber: true, hint: 'Your bank account number'),
                _buildInputField('IFSC Code *', _ifscCtrl,
                    isCaps: true, hint: 'e.g., SBIN0001234'),
                const SizedBox(height: 32),
  ...
  Widget _buildInputField(String label, TextEditingController controller,
      {bool isNumber = false, bool isCaps = false, String? hint}) {
    return Padding(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              hintText: hint,
              hintStyle:
                  GoogleFonts.outfit(color: Colors.white24, fontSize: 16),
              filled: true,
              ...
              focusedBorder: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(14),
                  borderSide: const BorderSide(color: Color(0xFF51CF66))),
              contentPadding:
        ...
        child: file != null
            ? const Center(
                child: Icon(Icons.check_circle_rounded,
                    color: Color(0xFF51CF66), size: 36))
            : Center(
                child: Column(
