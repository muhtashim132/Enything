-- =============================================================================
-- Migration: Remove Delivery Discount and Fix Financial Math
-- Description:
--   1. Drops delivery_discount column from orders.
--   2. Updates place_orders_transaction to bound rider_earnings mathematically
--      without needing delivery_discount.
--   3. Updates admin_get_finance_stats to subtract coupon_discount from pure_profit.
-- =============================================================================

ALTER TABLE orders DROP COLUMN IF EXISTS delivery_discount;
DELETE FROM platform_config WHERE key IN ('delivery_discount_threshold', 'delivery_discount_amount');

-- Fix 1: Update admin_get_finance_stats to subtract coupon_discount
...
-- Fix 2: Update place_orders_transaction to handle rider_earnings cleanly
...
BEGIN
  ...
  FOR v_order IN SELECT * FROM jsonb_to_recordset(p_orders) AS x(
    ...
    -- The frontend rider pay is based on Base Delivery + Surcharges (ignoring small cart fee and GST).
    -- By reconstructing the base from the total paid by the customer, we cap the maximum allowed pay
    -- to mathematically prevent siphoning.
    v_delivery_base := COALESCE(v_order.delivery_charges, 0) - COALESCE(v_order.gst_delivery, 0);
