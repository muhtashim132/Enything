-- =============================================================================
-- Migration: 100x Final Logic Fortress (Architecture Integrity Fixes)
-- Description:
--   1. Reverts shop_dispute from reallocate_cancelled_delivery_fees to 
--      prevent stealing delivery fees during temporary hold states.
--   2. Reverts the hard-zeroing of platform_fee in admin_cancel_order to 
--      restore receipt integrity (relying purely on refund_status for metrics).
-- =============================================================================

-- 1. Remove non-terminal `shop_dispute` from reallocation (and ensure verification_failed is absent)
...
BEGIN
    ...
    -- 100x FIX: 'shop_dispute' removed. 'verification_failed' excluded (rider gets paid for travel).
    SELECT 
...
-- 2. Restore Receipt Integrity in admin_cancel_order
