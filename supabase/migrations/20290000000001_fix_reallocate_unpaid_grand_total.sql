-- =============================================================================
-- Migration 20290000000001_fix_reallocate_unpaid_grand_total.sql
-- 
-- BUG FIX: reallocate_cancelled_delivery_fees was zeroing grand_total_collected
-- for ALL unpaid orders (ELSE 0 branch). This caused the Bill Summary to show
-- ₹0 for the remaining shops after a partial rejection, because:
--   1. reallocate_cancelled_delivery_fees is called when "Proceed & Pay" is tapped
--   2. The active orders have payment_status != 'captured' (not yet paid)
--   3. ELSE 0 sets grand_total_collected = 0
--   4. OrderModel.grandTotal getter returns grandTotalCollected when >= 0
--   5. So grandTotal = 0.0 ✗
--
-- FIX: ADDITIVE-ONLY — only the ELSE branch changes.
--      The WHEN 'captured' branch is UNCHANGED.
--      For unpaid active orders, compute the correct projected grand total
--      using the same formula as the captured branch.
-- =============================================================================
...
BEGIN
    ...
    -- 100x STRESS TEST FIX (Phase 24): Removed COALESCE(rider_earnings, 0) = 0 condition
    SELECT 
    ...
    IF v_missing_delivery > 0 THEN
        ...
        LOOP
            ...
            SET delivery_charges = 0,
                ...
                heavy_order_fee = 0,
                gst_delivery = 0,
                rider_earnings = 0, -- 100x STRESS TEST FIX (Phase 24): Explicitly zero out rider_earnings on cancelled order
    ...
    LOOP
        ...
        SET delivery_charges = rec.delivery_charges + v_split_delivery,
            ...
            -- BUG FIX 20290000000001: Previously ELSE 0 caused grand_total_collected = 0
            -- for all unpaid (awaiting_payment / awaiting_acceptance) active orders.
            -- OrderModel.grandTotal returns grandTotalCollected when >= 0, so it returned 0.
            -- Fix: compute the correct projected total for unpaid orders using the same
            -- formula as captured orders. Captured orders (already paid) are UNCHANGED.
            grand_total_collected = CASE 
                WHEN rec.payment_status = 'captured' THEN 
                    GREATEST(0, rec.total_amount + rec.gst_item_total + rec.platform_fee + rec.gst_platform + v_net_delivery - (rec.coupon_discount + v_split_coupon)) 
                ELSE 
                    -- ADDITIVE FIX: For unpaid orders, store the correct projected total
                    -- so the customer sees the right amount in the Bill Summary.
                    GREATEST(0, rec.total_amount + rec.gst_item_total + rec.platform_fee + rec.gst_platform + v_net_delivery - (rec.coupon_discount + v_split_coupon))
            END
        WHERE id = rec.id;
    END LOOP;
