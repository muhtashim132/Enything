-- Description: Fixes rider affinity for replacement orders and grand_total double-charging bug

-- 1. Patch get_nearby_unassigned_orders to enforce Exclusive Rider Affinity
...
-- 2. Fix reallocate_cancelled_delivery_fees grand_total bug and double-charging
    ...
    IF v_missing_delivery > 0 THEN
        ...
        LOOP
            ...
            SET delivery_charges = 0,
                ...
                heavy_order_fee = 0,
                gst_delivery = 0,
                grand_total_collected = GREATEST(0, rec.total_amount + rec.gst_item_total + rec.platform_fee + rec.gst_platform - COALESCE(coupon_discount, 0))
            WHERE id = rec.id;
        END LOOP;
    END IF;
...
-- 3. Fix rebalance_active_delivery_fees grand_total bug and double-charging
...
BEGIN
  ...
  LOOP
    ...
    SET delivery_charges = v_split_delivery,
        ...
        gst_delivery = v_new_gst_delivery,
        grand_total_collected = GREATEST(0, rec.total_amount + rec.gst_item_total + rec.platform_fee + rec.gst_platform + v_split_delivery - rec.coupon_discount)
    WHERE id = rec.id;
  END LOOP;
END;
$$;

-- 4. Restore the atomic cancellation block inside place_orders_transaction
