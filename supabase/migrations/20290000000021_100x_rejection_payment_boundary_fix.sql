-- Migration: 100x Rejection Payment Boundary Fix
-- Description: 
-- 1. Groups reallocate_cancelled_delivery_fees and rebalance_active_delivery_fees 
--    strictly by razorpay_payment_id. This completely eliminates Razorpay refund 
--    crashes ("Refund amount exceeds payment amount") when customers replace 
--    partially rejected orders.
-- 2. Halts delivery fee stripping if all orders in a payment are cancelled, 
--    preventing single-shop cancellation delivery fee theft.
