-- =============================================================================
-- Migration: 20290000000022_100x_additive_delivery_floor_bypass.sql
-- Description: ADDITIVE ONLY — CREATE OR REPLACE FUNCTION only.
--              Bypasses the Delivery charge floor breached check for 
--              additive orders (when a customer adds items to a partially 
--              rejected order). This ensures they aren't incorrectly blocked 
--              when the new sub-order has a legitimately zero delivery charge 
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_existing_group_orders FROM orders WHERE cart_group_id = p_cart_group_id AND status NOT IN ('cancelled', 'payment_failed', 'timeout', 'seller_rejected', 'shop_dispute_cancel');
  ...
  LOOP
    ...
    IF v_existing_group_orders = 0 THEN
