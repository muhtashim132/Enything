-- =============================================================================
-- Migration: 100x Payment Bypass Fortress (Zero-Day Patches)
-- Description:
--   1. Patches the "Free Money" Payment Bypass Exploit: Revokes EXECUTE on 
--      `client_confirm_payment` from authenticated/anon/public, restricting it 
--      exclusively to `service_role`. This forces all payments to be verified 
--      via the Edge Function HMAC-SHA256 signature check.
--   2. Patches the Global DoS Order Cancellation Exploit: Adds a strict 
--      authorization check to `cancel_order` so malicious users cannot arbitrarily 
--      cancel other customers' pending orders.
--   3. Drops obsolete dangerous RPCs (`decrement_product_stock`, etc).
-- =============================================================================

-- 1. Patch Payment Bypass (Restrict to service_role)
REVOKE EXECUTE ON FUNCTION client_confirm_payment(UUID, UUID, text, text) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION client_confirm_payment(UUID, UUID, text, text) TO service_role;

-- 2. Patch Global DoS Order Cancellation
...
-- 3. Drop obsolete dangerous RPCs that could be exploited
