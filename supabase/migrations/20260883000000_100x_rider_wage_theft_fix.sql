-- =============================================================================
-- Phase 28: Rider Wage Theft & Double Surcharge Calculation Fix
-- Description: The frontend includes small_cart_fee, heavy_order_fee, and 
-- multi_shop_surcharge inside `delivery_charges`. The backend was double-counting 
-- them in both `grand_total` and `rider_earnings` calculation. Fixed math to pull
-- directly from pure payloads and avoid inflating payments/deductions.
-- =============================================================================
...
BEGIN
  ...
  v_global_delivery_rate_per_km := COALESCE(v_global_delivery_rate_per_km, 20.0);
  ...
  IF p_coupon_id IS NOT NULL THEN
    ...
    FROM coupons WHERE id = p_coupon_id;
...
BEGIN
    ...
    LOOP
        ...
        SET delivery_charges = rec.delivery_charges + v_split_delivery,
            -- DYNAMIC COMMISSION (Surcharges already inside delivery_charges)
