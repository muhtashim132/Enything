-- =============================================================================
-- Migration: 20290000000117_100x_cascading_rejection_and_contract_resilience.sql
-- =============================================================================
-- Description:
--   1. Fixes the Cascading Rejection Financial Bug in `reallocate_cancelled_delivery_fees`:
--      - When multiple shops in a multi-shop cart group cancel sequentially, previous
--        cancellations are finalized (fees zeroed out) and their `grand_total_collected`
--        is permanently preserved and NEVER overwritten.
--      - Only newly cancelled orders (with delivery_charges > 0 OR platform_fee > 0)
--        participate in the incremental refund distribution.
--      - Seamlessly handles `v_active_count = 0` when all shops cancel, ensuring zero
--        customer funds are lost.
--      - Adds deterministic row-level locking (`ORDER BY id FOR UPDATE`) to guarantee
--        absolute serializability and eliminate concurrency deadlocks.
--   2. Fortifies `place_orders_transaction`:
--      - Enforces `cancelled_reason = 'customer_replaced'` when `p_order_id_to_cancel`
--        is provided to preserve contract with track_order_page and auto-cancel crons.
--      - Reallocates delivery fees & rebalances active orders atomically when replacement
--        orders are placed into an existing cart group.
--      - Adds failsafe fallback for `acceptance_deadline` (NOW() + 3 minutes) if null/omitted.
-- =============================================================================
...
DECLARE
  ...
  v_remaining_max_dist_km NUMERIC := 3.0;
...
BEGIN
    -- 100x FIX: Deterministic row-level locking to guarantee serializability & eliminate concurrency deadlocks
    ...
    LOOP
        ...
        -- Count newly cancelled orders (cancelled orders that still have delivery_charges > 0 OR platform_fee > 0)
        SELECT COUNT(id)
        ...
        -- If no active orders AND no newly cancelled orders, nothing to do
        IF v_active_count = 0 AND v_newly_cancelled_count = 0 THEN
            CONTINUE;
        END IF;

        -- CASE A: ALL SHOPS ARE NOW CANCELLED (v_active_count = 0)
        IF v_active_count = 0 THEN
            -- Finalize any newly cancelled orders: zero out fees, keep their existing grand_total_collected as the refund
        ...
        -- CASE B: ACTIVE SHOPS REMAIN (v_active_count > 0)
        -- If no new cancellations need refund allocation, simply rebalance active orders
        ...
        -- Available pool is strictly the money from active orders PLUS newly-cancelled orders
        -- Already-finalized cancelled orders (delivery_charges = 0 AND platform_fee = 0) are excluded so their refunds are NEVER corrupted!
        SELECT 
        ...
        -- DYNAMIC DISTANCE RECALCULATION FOR REMAINING ACTIVE SHOPS:
        SELECT COALESCE(MAX(estimated_distance_km), 3.0)
          INTO v_remaining_max_dist_km
          FROM public.orders
         WHERE cart_group_id = p_cart_group_id
           AND razorpay_payment_id IS NOT DISTINCT FROM pay_rec.razorpay_payment_id
           AND status NOT IN ('cancelled', 'seller_rejected', 'partner_rejected', 'rider_rejected', 'verification_failed', 'timeout', 'payment_failed', 'shop_dispute_cancel');

        v_new_base_delivery := GREATEST(v_admin_delivery_base, GREATEST(1, CEIL(v_remaining_max_dist_km)) * v_delivery_rate_per_km);
        ...
        -- Recalculated total active delivery fee
        v_new_active_delivery_total := (v_new_base_delivery + v_allowed_surcharge + (v_new_small * v_active_count) + (v_new_heavy * v_active_count)) * (1.0 + v_delivery_gst_rate);
        v_total_cart_delivery := LEAST(v_total_cart_delivery, v_new_active_delivery_total);
        ...
        -- Sum current grand_total of remaining active orders
        ...
        -- The exact total refund amount released by this cancellation event
        v_total_refund_amount := GREATEST(0, v_available_pool - v_sum_active_grand);

        -- Sum previous grand_total_collected of the NEWLY cancelled orders to distribute refund proportionately
        SELECT COALESCE(SUM(grand_total_collected), 0)
        ...
        -- Distribute refund across NEWLY cancelled orders ONLY
        FOR c_rec IN
        ...
        LOOP
            ...
            IF v_cancelled_idx = v_newly_cancelled_count THEN
                -- Last newly cancelled order gets exact remainder to eliminate any floating-point/paisa mismatch
                v_order_refund := v_total_refund_amount - v_allocated_refund_sum;
            ELSIF v_sum_cancelled_weights > 0 THEN
                v_order_refund := v_total_refund_amount * (c_rec.grand_total_collected / v_sum_cancelled_weights);
                v_allocated_refund_sum := v_allocated_refund_sum + v_order_refund;
            ELSE
                v_order_refund := v_total_refund_amount / v_newly_cancelled_count;
                v_allocated_refund_sum := v_allocated_refund_sum + v_order_refund;
            END IF;

            -- Update THIS newly cancelled order with its refund and zero out fees
            UPDATE public.orders
               SET grand_total_collected = GREATEST(0, v_order_refund),
                   delivery_charges      = 0,
                   platform_fee          = 0,
                   small_cart_fee        = 0,
                   heavy_order_fee       = 0,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Fortified place_orders_transaction
-- ─────────────────────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_max_cart_dist_km double precision := 0.0;
  ...
  v_expected_delivery_fee numeric := 0.0;
  v_expected_platform_fee numeric := 20.0;
  
  v_sum_client_multi_shop_surcharge numeric := 0.0;
  v_sum_client_heavy_order_fee numeric := 0.0;
  v_sum_client_small_cart_fee numeric := 0.0;
  v_sum_client_platform_fee numeric := 0.0;
  v_sum_client_delivery_charges numeric := 0.0;
  v_sum_expected_total_amount numeric := 0.0;
  
  v_total_weight_kg numeric := 0.0;
  ...
  v_global_heavy_order_fee numeric := 20.0;
...
BEGIN
  ...
  IF p_orders IS NULL OR jsonb_array_length(p_orders) = 0 THEN
    RAISE EXCEPTION 'Cannot place order: orders list is empty.';
  END IF;

  IF p_items IS NULL OR jsonb_array_length(p_items) = 0 THEN
    RAISE EXCEPTION 'Cannot place order: items list is empty.';
  END IF;

  IF p_cart_group_id IS NULL THEN
    RAISE EXCEPTION 'Cannot place order: cart_group_id is missing.';
  END IF;

  -- 100x Idempotency: Protect against duplicate checkouts
  IF p_idempotency_key IS NOT NULL AND trim(p_idempotency_key) <> '' THEN
    PERFORM pg_advisory_xact_lock(hashtext('idempotency_lock_' || p_idempotency_key));

    SELECT array_agg(id) INTO v_inserted_ids
    FROM orders
    WHERE idempotency_key::text = p_idempotency_key::text;

    IF v_inserted_ids IS NOT NULL AND array_length(v_inserted_ids, 1) > 0 THEN
      RETURN jsonb_build_object(
        'success', true,
        'message', 'Order already placed (idempotent)',
        'order_ids', (SELECT jsonb_agg(id) FROM orders WHERE idempotency_key::text = p_idempotency_key::text)
      );
    END IF;
  END IF;
  ...
  IF p_order_id_to_cancel IS NOT NULL THEN
    ...
    -- For already terminal orders, preserve status and set contractual tag
    UPDATE orders 
  ...
  -- If replacement order, inherit already assigned delivery partner
  ...
  -- Fetch dynamic platform rates safely
  ...
  -- Geospatial validation & cart maximum distance determination
  FOR v_order IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    IF v_shop_lat IS NOT NULL AND v_shop_lng IS NOT NULL 
       ...
       BEGIN
         ...
         -- Prevent floating-point boundary jitter near integer km boundaries
    ...
    v_max_cart_dist_km := GREATEST(v_max_cart_dist_km, COALESCE(v_dist_km, 0.0));
  END LOOP;
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(
    ...
    ELSIF v_item.weight_kg IS NOT NULL AND v_item.weight_kg > 0 THEN
      v_total_weight_kg := v_total_weight_kg + (v_item.weight_kg * v_item.quantity);
    ELSE
  ...
  -- 5. Validate Delivery Fee (Dynamic Per-Km Engine)
  IF v_is_replacement THEN
  ...
  ELSE
    v_calculated_delivery_base := GREATEST(
      ...
      GREATEST(1, CEIL(v_max_cart_dist_km)) * v_delivery_rate_per_km
    );
    v_expected_delivery_fee := (v_calculated_delivery_base + v_sum_client_multi_shop_surcharge + v_sum_client_heavy_order_fee + v_sum_client_small_cart_fee) * (1.0 + v_delivery_gst_rate);
  END IF;
  ...
  FOR v_order IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(product_id uuid, variant_name text, price numeric, quantity int, order_id uuid) WHERE order_id = (v_order->>'id')::uuid LOOP
      ...
      v_expected_total_amount := v_expected_total_amount + (v_item.price * v_item.quantity);
      ...
      -- Income Tax TDS (§194-O: 0.1% of gross sales for e-commerce sellers)
    ...
    -- Seller absorbs gateway fee (2.36%) only on their payout portion
    ...
    -- 100x FIX: Absolute fallback for acceptance deadline (NOW() + 3 minutes) if null or omitted
    v_acceptance_deadline := COALESCE((v_order->>'acceptance_deadline')::timestamptz, NOW() + INTERVAL '3 minutes');
    ...
    INSERT INTO orders (
      ...
      estimated_distance_km, cancelled_reason, customer_phone, shop_phone,
      shop_prep_time_snapshot, prescription_urls, gst_rate_snapshot,
      razorpay_payment_id, razorpay_order_id, cart_group_id,
      total_amount, gst_item_total, platform_fee, delivery_charges,
      multi_shop_surcharge, small_cart_fee, heavy_order_fee, coupon_discount,
      grand_total_collected, s9_5_gst_amount, non_food_gst_amount,
      gst_platform, gst_delivery, tcs_amount, tds_amount, gateway_deduction,
      seller_payout, enything_commission, rider_earnings, coupon_id,
      idempotency_key, acceptance_deadline
    ) VALUES (
      ...
      COALESCE((v_secure_order->>'estimated_distance_km')::numeric, 0),
      v_secure_order->>'cancelled_reason',
      v_secure_order->>'customer_phone',
      v_secure_order->>'shop_phone',
      COALESCE((v_secure_order->>'shop_prep_time_snapshot')::integer, 30),
      COALESCE(v_secure_order->'prescription_urls', '[]'::jsonb),
      COALESCE(v_secure_order->'gst_rate_snapshot', '{}'::jsonb),
  ...
  -- Inventory reservation & soft delete guard (Rule: is_deleted = false)
  FOR v_item IN
  ...
  -- 100x FIX: Atomically reallocate fees and rebalance active orders when replacement order is placed
  IF p_order_id_to_cancel IS NOT NULL THEN
...
GRANT EXECUTE ON FUNCTION public.place_orders_transaction(jsonb, jsonb, uuid, uuid, text, uuid) TO authenticated, service_role;
