-- =============================================================================
-- Migration: 20290000000088_100x_flawless_order_and_financial_fortress.sql
-- =============================================================================
-- Description:
--   ADDITIVE ONLY & FLAWLESS FINANCIAL FORTIFICATION:
--   1. Enhances place_orders_transaction() to preserve the client's category GST map
--      in gst_rate_snapshot (jsonb) rather than collapsing to a scalar.
--   2. Properly apportions gateway deduction in seller_payout calculation according
--      to TaxConfig (seller pays gateway share only on their subtotal + non-food GST).
--   3. Validates both grand_total_collected and grand_total if present in order payload.
--   4. Preserves 100% of all soft-delete guards, row-level locks, stock checks,
--      multi-shop surcharge checks, and replacement order logic.
-- =============================================================================
