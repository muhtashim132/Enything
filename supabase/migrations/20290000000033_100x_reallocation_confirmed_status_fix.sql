-- =============================================================================
-- Migration: 100x Reallocation Confirmed Status Mathematical Freeze Fix
-- Description:
--   Fixes a critical flaw where `reallocate_cancelled_delivery_fees` mathematically 
--   ignored active shops that were in 'confirmed' or 'pending' states because they 
--   were missing from the explicit `status IN (...)` lists.
--   This caused partial rejections to silently bypass fee absorption, leaving 
--   phantom fees (like +1 multi-shop surcharge and handling fee offsets) 
--   mathematically trapped in the UI and DB for the remaining confirmed shop.
--   We now use a robust `NOT IN (...)` check to guarantee all active states are caught.
-- =============================================================================
