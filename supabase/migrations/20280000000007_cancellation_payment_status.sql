-- Migration: 20280000000007_cancellation_payment_status.sql
-- Description: Fixes customer notifications for cancelled orders when payment has already been captured.
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Shop Accepted notification (Fires independently of status changes)
    IF NEW.seller_accepted = true AND OLD.seller_accepted = false THEN
    ...
    IF NEW.status != OLD.status THEN
      ...
      CASE NEW.status
        ...
        WHEN 'cancelled' THEN
          ...
          IF NEW.payment_status = 'captured' OR NEW.payment_status = 'paid' THEN
            v_body := 'Your order has been cancelled. A refund will be processed.';
          ELSE
        ...
        WHEN 'seller_rejected' THEN
          ...
          IF NEW.payment_status = 'captured' OR NEW.payment_status = 'paid' THEN
            v_body := COALESCE(v_shop_name, 'The shop') || ' could not accept your order. A refund will be processed.';
          ELSE
        ...
        ELSE
          -- Do nothing (Removed awaiting_payment notification to allow silent auto-redirect)
      END CASE;
