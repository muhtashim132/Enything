-- =============================================================================
-- Migration: 100x Rejection Extreme Edge Fortress (Phase 2)
-- Description:
--   1. Fixes N+1 Query Overload by replacing the `FOR` loop with deterministic 
--      bulk locking (`PERFORM ... FOR UPDATE`) followed by a bulk `UPDATE`.
--   2. Fixes a NULL Authorization Bypass where `NULL != auth.uid()` evaluated 
--      to NULL instead of TRUE, allowing unauthorized rejection of orphaned shops.
-- =============================================================================
...
BEGIN
  ...
  -- 100x STRESS TEST FIX: Prevent NULL Privilege Escalation Bypass
  -- `v_seller_id != auth.uid()` evaluates to NULL (falsy) if v_seller_id is NULL.
  ...
  -- STRESS TEST FIX: N+1 Exhaustion AND Deadlocks eliminated simultaneously
