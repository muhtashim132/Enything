-- =============================================================================
-- Migration: 100x Admin Cross-Role NULL Bypass Fix
-- Description:
--   Seals a catastrophic cascading ternary logic vulnerability where 
--   SQL's `!=` operator evaluated to `NULL` (resolving to `FALSE` in PL/pgSQL 
--   `IF` conditions) when a target ID (like `delivery_partner_id`) was missing.
--   This flaw allowed malicious users to bypass IDOR guards and execute state 
--   mutations (like picking up an unassigned order).
--
--   Fix: Replaced all `!=` checks with `IS DISTINCT FROM` to ensure strict 
--   boolean resolution (TRUE/FALSE) regardless of NULL operands.
-- =============================================================================

-- 1. Seal Admin Access to Seller Balance
...
BEGIN
  -- 100x FIX: Seal Ternary NULL Bypass with IS DISTINCT FROM
...
-- 2. Seal Admin Access to Rider Balance
...
BEGIN
  -- 100x FIX: Seal Ternary NULL Bypass with IS DISTINCT FROM
  IF (auth.uid() IS NULL OR auth.uid() IS DISTINCT FROM p_rider_id) AND NOT public.is_active_admin(auth.uid()) THEN
    RAISE EXCEPTION 'Unauthorized: Cannot access financial data for another user.';
  END IF;
...
-- 3. Seal Admin Access to CA Report
...
-- 4. Seal Admin Force-Update Capability in Order Status
...
BEGIN
  ...
  IF p_new_status IN ('preparing', 'ready_for_pickup') THEN
    ...
    -- 100x FIX: Seal Ternary NULL Bypass with IS DISTINCT FROM
    IF (v_seller_id IS DISTINCT FROM auth.uid() OR auth.uid() IS NULL) AND NOT public.is_active_admin(auth.uid()) THEN
  ...
  ELSIF p_new_status IN ('picked_up', 'out_for_delivery', 'delivered') THEN
    -- 100x FIX: Seal Ternary NULL Bypass with IS DISTINCT FROM
    IF (v_rider_id IS DISTINCT FROM auth.uid() OR auth.uid() IS NULL) AND NOT public.is_active_admin(auth.uid()) THEN
...
-- 5. Seal Missing Customer Dashboard Stats RPC
...
BEGIN
  -- 100x FIX: Seal Ternary NULL Bypass with IS DISTINCT FROM
  IF (auth.uid() IS NULL OR auth.uid() IS DISTINCT FROM p_customer_id) AND NOT public.is_active_admin(auth.uid()) THEN
