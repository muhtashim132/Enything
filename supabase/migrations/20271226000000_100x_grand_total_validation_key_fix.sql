-- =============================================================================
-- Migration: 20271226000000_100x_grand_total_validation_key_fix.sql
-- Description: ADDITIVE ONLY — CREATE OR REPLACE FUNCTION only.
--              Fixes a dormant (dead-code) security validation in
--              place_orders_transaction that has existed since the very first
--              version of the function (20260896000000).
--
-- THE BUG (present in ALL 30+ prior versions of place_orders_transaction):
--   Line 417 reads: (v_order->>'grand_total')::numeric
--   Flutter sends:  'grand_total_collected'   (NEVER 'grand_total')
--
--   Since v_order->>'grand_total' = NULL always:
--     ABS(NULL - v_expected_grand_total) = NULL
--     NULL > 1.0 = NULL (falsy in PL/pgSQL)
--   → The grand total spoofing check NEVER runs. It is dead code.
--
--   Data integrity is NOT broken (INSERT always uses server-computed
--   v_expected_grand_total, never the client value). But the anti-spoofing
--   check is meaningless — any client-supplied grand_total_collected passes.
--
-- THE FIX:
--   Change 'grand_total' → 'grand_total_collected' in the validation check.
--   Add IS NOT NULL guard so that if grand_total_collected is genuinely
--   absent from the JSON (legacy client edge case), the check passes silently
--   (identical behavior to before). The INSERT always uses server value.
--
-- IMPACT ON ALL USERS:
--   Customer (checkout_page.dart:716)    → sends 'grand_total_collected' ✓
--   Customer (track_order_page.dart:943) → sends 'grand_total_collected' ✓
--   Seller  → does NOT call this RPC ✓
--   Rider   → does NOT call this RPC ✓
--   Admin   → does NOT call this RPC ✓
--   magic_reviewer_auto_accept → bypasses this RPC ✓
--   dev_client_confirm_payment → bypasses this RPC ✓
--
-- SAFETY VERIFICATION:
