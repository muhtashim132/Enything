-- =============================================================================
-- Migration: 100x Ghost Surcharge and Handling Fee UI Fix
-- Description:
--   1. Ghost Surcharge Fix: Dynamically subtracts the cancelled Multi-shop 
--      Surcharge (plus GST) from the master `delivery_charges` pool, preventing 
--      the active shop's Delivery Fee UI from absorbing the cancelled ghost fee.
--   2. Handling Fee Reversal: Dynamically recalculates the Platform Fee based 
--      on the remaining active shops, ensuring it reverts from a multiplied 
--      value (e.g. 40) down to the base config value (e.g. 20).
-- =============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- Calculate the cancelled surcharge (which is mathematically hiding inside delivery_charges + GST)
        v_cancelled_surcharge := GREATEST(0, v_original_surcharge - v_allowed_surcharge);
        
        -- 100x GHOST SURCHARGE FIX: Remove the cancelled surcharge (and its GST) from the master delivery pool
        v_total_cart_delivery := GREATEST(0, v_total_cart_delivery - (v_cancelled_surcharge * (1.0 + v_delivery_gst_rate)));
        
        -- 100x HANDLING FEE REVERSAL FIX: Recalculate platform fee based on remaining active shops
