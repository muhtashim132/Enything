-- =============================================================================
-- Migration: 100x Checkout Delivery Math & Double-Counting Fix
-- Description:
--   1. Fixes the delivery fee verification formula to match the Dart app (includes GST, heavy/small/multi-shop fees).
--   2. Fixes v_expected_grand_total to NOT double-count fees that are already inside delivery_charges.
-- =============================================================================
...
BEGIN
  ...
  v_global_heavy_order_fee := COALESCE(v_global_heavy_order_fee, 10.0) * GREATEST(0, CEIL(v_total_weight_kg - v_global_heavy_order_threshold));
  ...
  BEGIN SELECT value::numeric INTO v_expected_multi_shop_surcharge FROM platform_config WHERE key = 'multi_shop_surcharge'; EXCEPTION WHEN OTHERS THEN v_expected_multi_shop_surcharge := 15.0; END;
  v_expected_multi_shop_surcharge := COALESCE(v_expected_multi_shop_surcharge, 15.0);
  
  IF v_num_shops > 1 THEN
    IF ABS(v_sum_client_multi_shop_surcharge - v_expected_multi_shop_surcharge) > 1.0 THEN
      RAISE EXCEPTION 'Multi-shop surcharge spoofing detected. Expected: %, Got: %', v_expected_multi_shop_surcharge, v_sum_client_multi_shop_surcharge;
    END IF;
  ELSE
  ...
  v_expected_delivery_fee := CEIL(v_cart_distance_km) * v_delivery_rate_per_km;
  IF v_expected_delivery_fee < 25.0 THEN v_expected_delivery_fee := 25.0; END IF;
  ...
  FOR v_order_record IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    -- riderEarnings = (delivery_charges_without_gst) * riderPayoutRatio
    v_server_rider_earnings := (COALESCE((v_order->>'delivery_charges')::numeric, 0) - v_server_gst_delivery) * 0.80;
