-- =============================================================================
-- Migration: 100x Rejection Menu Annihilation Fortress (Phase 6)
-- Description:
--   1. Fixes a Catastrophic Menu Annihilation Exploit where a seller rejecting 
--      a massive multi-item order for "Out of Stock" would maliciously take 
--      ALL items in the order offline (bricking the shop's menu).
--   2. Restricts the auto-disable Ghost Inventory Loop fix to strictly apply 
--      ONLY when the order contains exactly ONE distinct product.
-- =============================================================================
...
BEGIN
  ...
  -- 100x STRESS TEST FIX (Phase 6): Prevent Menu Annihilation by bounding to single-item orders
  IF p_reject_reason = 'out_of_stock' AND (SELECT COUNT(DISTINCT product_id) FROM order_items WHERE order_id = p_order_id) = 1 THEN
  ...
  SET 
    ...
    -- 100x STRESS TEST FIX (Phase 4): Prevent Double-Refund Financial Blackhole Exploit
    refund_status = CASE 
