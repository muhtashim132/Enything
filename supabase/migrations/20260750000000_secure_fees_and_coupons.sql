-- =============================================================================
-- Migration: Secure Fees and Coupons Math (100x Backend Security)
-- Description:
--   1. Validates that the sum of platform_fee, small_cart_fee, heavy_order_fee
--      matches the server's platform_config exactly.
--   2. Verifies coupon discount mathematically against the coupon definition.
--   3. Recalculates gst_platform and gst_delivery securely on the backend,
...
BEGIN
  ...
  -- 2. Aggregate Client Fees & Calculate Expected Total Amount
  ...
  FOR v_item IN SELECT y.quantity, y.price FROM jsonb_to_recordset(p_items) AS y(quantity int, price numeric) LOOP
  ...
  -- Provide a 1 rupee buffer for floating point apportionment splitting in Dart
  ...
  IF v_sum_client_small_cart_fee > 0 AND ABS(v_sum_client_small_cart_fee - v_global_small_cart_fee) > 1.0 THEN
    RAISE EXCEPTION 'Small cart fee spoofing detected. Expected: 0 or %, Got: %', v_global_small_cart_fee, v_sum_client_small_cart_fee;
  END IF;

  IF v_sum_client_heavy_order_fee > 0 AND ABS(v_sum_client_heavy_order_fee - v_global_heavy_order_fee) > 1.0 THEN
    RAISE EXCEPTION 'Heavy order fee spoofing detected. Expected: 0 or %, Got: %', v_global_heavy_order_fee, v_sum_client_heavy_order_fee;
  END IF;
  ...
  IF p_coupon_id IS NOT NULL THEN
    ...
    -- Ensure client did not claim more discount than mathematically possible
  ...
  FOR v_order IN SELECT * FROM jsonb_to_recordset(p_orders) AS x(
    ...
    -- Override GST Platform and GST Delivery securely using dynamic DB rates
    ...
    -- Rider Earnings Math & Spoofing Fix
    ...
    IF COALESCE(v_order.rider_earnings, 0) > (v_expected_rider_base * 0.80) + 1.0 THEN
        RAISE EXCEPTION 'Rider earnings spoofing detected.';
    END IF;

    v_server_rider_earnings := COALESCE(v_order.rider_earnings, 0);
    ...
    -- Strip delivery_discount from secure order if client sent it anyway
