-- =============================================================================
-- Migration: 100x State Machine Resurrection Fix
-- Description:
--   1. Patches `update_order_status` to strictly prevent sellers from resurrecting 
--      terminal/refunded orders back to 'preparing' or 'ready_for_pickup'.
--   2. Patches `request_rider_withdrawal` to ensure riders cannot mathematically 
--      extract earnings from orders that were refunded or cancelled.
-- =============================================================================

-- 1. Fix State Machine Resurrection Exploit in update_order_status
...
BEGIN
  ...
  -- 100x FIX: Enforce STRICT PREVIOUS STATE constraints for sellers to prevent terminal state resurrection
...
-- 2. Close the Refund Blackhole in request_rider_withdrawal
...
BEGIN
  ...
  -- 100x FIX: Zero out rider earnings if the order was refunded (Refund Blackhole Protection)
  SELECT COALESCE(SUM(
    CASE 
      WHEN COALESCE(refund_status, 'none') IN ('processing', 'completed') THEN 0 
      ELSE COALESCE(rider_earnings, 0) + COALESCE(wait_time_penalty, 0)
    END
