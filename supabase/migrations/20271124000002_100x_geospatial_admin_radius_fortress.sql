...
-- Additive RPC for Phase 36: DB OOM & PostGIS Starvation Eradication
-- We are enforcing LEAST() caps on p_radius_km and p_limit to prevent malicious API requests from taking down the PostgreSQL instance.
-- This preserves the existing is_accepting_orders logic from Phase 31.
...
-- Description: ADDITIVE ONLY â€” new read-only RPC that returns the most-ordered
--              product names from the last 30 days of delivered orders,
--              filtered by an admin-defined delivery radius.
--
-- ADDITIVE GUARANTEE:
--   â€¢ CREATE OR REPLACE FUNCTION (new function, does not modify any existing one)
--   â€¢ No ALTER TABLE, no DROP, no trigger changes, no policy changes
--   â€¢ Uses existing ST_DWithin logic validated in search geospatial RPCs
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(v_order_totals.estimated_distance_km, 0) > v_global_max_radius THEN
      RAISE EXCEPTION 'Distance spoofing detected (Money Laundering Exploit). The maximum allowed delivery radius is % km. Claimed: % km', v_global_max_radius, v_order_totals.estimated_distance_km;
    END IF;
