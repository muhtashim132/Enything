-- =============================================================================
-- Migration: 100x Extreme Edge Case Hoarding Fixes (Phase 3)
-- Description:
--   1. Fixes the massive NULL regression where COUNT(DISTINCT cart_group_id) 
--      ignored NULL values, allowing riders to hoard infinite orders if 
--      those orders lacked a cart group.
--   2. Injects COALESCE(cart_group_id, id) across both overloaded functions
--      to guarantee isolated orders are strictly counted as their own group.
-- =============================================================================
...
DECLARE
  ...
  v_active_carts int;
BEGIN
  -- 100x ARCHITECTURE FIX: Apply strict advisory lock
  PERFORM pg_advisory_xact_lock(hashtext('rider_acceptance_' || COALESCE(auth.uid()::text, 'system_admin')));

  -- 100x FIX: Enforce MAX 3 Active Cart Groups with COALESCE NULL Protection
