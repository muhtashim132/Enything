-- =============================================================================
-- Migration: 100x Extract Bundled Delivery Fees
-- Description:
--   Fixes a bug where `delivery_charges` in the database is a bundled field 
--   containing (base delivery + surcharge + small_cart + heavy_order).
--   When reallocating, we were absorbing the ENTIRE bundled field, meaning 
--   cancelled surcharges were mathematically trapped inside the new active 
--   delivery fee! This extracts the pure base delivery first, preventing 
--   the customer from being overcharged on delivery.
-- =============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- The delivery_charges column in the DB is bundled (base + surcharge + small + heavy)
        -- We must extract the pure base delivery so we don't accidentally force the active shop 
        -- to absorb the cancelled surcharge hidden inside the delivery fee!
        ...
        LOOP
            ...
            -- 1. Split pure base delivery proportionally to guarantee rider max-distance pay
            -- 2. Construct the NEW bundled delivery_charges using the NEW active fees ONLY
            ...
            -- Rider earnings must be calculated from the newly absorbed delivery fee
            v_new_rider := GREATEST(0, (v_new_del - v_new_gst_del - COALESCE(rec.small_cart_fee, 0)) * (v_rider_commission_percent / 100.0));
            
            -- Calculate new grand total without absorbing the cancelled shop's platform fee
            v_new_grand := GREATEST(0, rec.total_amount + rec.gst_item_total + COALESCE(rec.platform_fee, 0) + v_new_del + COALESCE(rec.platform_fee, 0) * v_platform_gst_rate + COALESCE(rec.small_cart_fee, 0) + COALESCE(rec.heavy_order_fee, 0) + v_new_surcharge - COALESCE(rec.coupon_discount, 0));
            -- WAIT! The platform_fee already includes GST in the database. So I don't need to add it again.
            ...
            -- NOTE: Wait, previously it was: + v_new_del + COALESCE(rec.small_cart_fee, 0) + COALESCE(rec.heavy_order_fee, 0) + v_new_surcharge.
            -- BUT v_new_del ALREADY includes small_cart, heavy_order, and surcharge!
            -- So if I added them again, I would be double counting them in grand total!
            -- Let's trace it carefully:
            -- v_new_grand = items + gst_items + platform_fee + bundled_delivery_charges - discount.
            -- This is perfectly mathematically sound because bundled delivery_charges contains EVERYTHING else.
