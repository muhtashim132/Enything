-- =============================================================================
-- Migration: 100x TOCTOU Concurrency Fix
-- Description:
--   1. Fixes TOCTOU Razorpay Double-Spend exploit in client_confirm_payment by 
--      using an exclusive pg_advisory_xact_lock on the payment ID.
--   2. Fixes TOCTOU Phantom Order Exploit in place_orders_transaction by
--      using an exclusive pg_advisory_xact_lock on the idempotency key.
-- =============================================================================

-- 1. Patch client_confirm_payment for Double-Spend TOCTOU protection
...
-- 2. Patch place_orders_transaction for TOCTOU Idempotency bypass
...
BEGIN
  ...
  -- 100x FIX: TOCTOU Idempotency Race Condition Prevention
  -- Without this lock, concurrent identical requests can bypass the IF EXISTS check and duplicate orders.
  ...
  BEGIN SELECT value::numeric INTO v_delivery_base_fee FROM platform_config WHERE key = 'delivery_base_fee'; EXCEPTION WHEN OTHERS THEN v_delivery_base_fee := 10.0; END;
  v_delivery_base_fee := COALESCE(v_delivery_base_fee, 10.0);

  BEGIN SELECT value::numeric INTO v_delivery_rate_per_km FROM platform_config WHERE key = 'delivery_rate_per_km'; EXCEPTION WHEN OTHERS THEN v_delivery_rate_per_km := 5.0; END;
