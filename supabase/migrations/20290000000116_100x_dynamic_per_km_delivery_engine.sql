-- =============================================================================
-- Migration: 20290000000116_100x_dynamic_per_km_delivery_engine.sql
-- =============================================================================
-- Description:
--   1. Replaces flat delivery validation in place_orders_transaction() with a true
--      dynamic per-km delivery engine:
--      v_calculated_delivery_base := GREATEST(v_global_delivery_base, GREATEST(1, CEIL(v_max_cart_dist_km)) * v_delivery_rate_per_km);
--      Where:
--        - v_max_cart_dist_km is the furthest shop distance in the cart group
--        - v_delivery_rate_per_km is read dynamically from platform_config (default 20.0)
--        - v_global_delivery_base is the minimum base floor from platform_config (default 20.0)
--
--   2. Preserves consolidated multi-shop delivery charge:
--      Single cart delivery = Cart Base Delivery + Multi-Shop Surcharge (for N > 1 shops).
--      Distributed evenly across shop order records under the same cart_group_id.
--
--   3. Fortifies partial order rejection (reallocate_cancelled_delivery_fees & rebalance_active_delivery_fees):
--      - Recalculates multi-shop surcharge for remaining active shops.
--      - Recalculates distance-based delivery fee if furthest shop was cancelled.
--      - Accurately assigns refund to cancelled order(s) for automatic gateway refund.
--
--   4. Preserves 100% of all existing architecture:
--      (Replacement order zero base delivery/platform fee, rider inheritance,
--       WeightEngine alignment, gateway deduction 18% GST, soft delete guard,
--       idempotency key text safety, TCS/TDS statutory deductions).
-- Ensure orders.idempotency_key column is TEXT to match p_idempotency_key text
ALTER TABLE public.orders ALTER COLUMN idempotency_key TYPE text USING idempotency_key::text;
...
DECLARE
  ...
  -- Fixed gateway GST rate (Razorpay fee GST is 18%)
...
BEGIN
  IF p_idempotency_key IS NOT NULL THEN
    ...
    IF EXISTS (SELECT 1 FROM orders WHERE idempotency_key::text = p_idempotency_key::text) THEN
  ...
  IF p_order_id_to_cancel IS NOT NULL THEN
    ...
    -- For already terminal orders, DO NOT touch status
    UPDATE orders 
  ...
  -- Read Dynamic Platform Configuration
  ...
  FOR v_order IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    IF v_shop_lat IS NOT NULL AND v_shop_lng IS NOT NULL 
       ...
       BEGIN
         ...
         -- If client's estimated distance is within 0.3 km of server-computed distance, honor client distance
  ...
  -- Inventory reservation & soft delete guard
  FOR v_item IN
...
-- ─────────────────────────────────────────────────────────────────────────────
-- reallocate_cancelled_delivery_fees — Dynamic per-km & surcharge reallocation
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- rebalance_active_delivery_fees — Equal split rebalancer for active orders
-- ─────────────────────────────────────────────────────────────────────────────
