-- Migration: 100x Partial Rejection Math Correction
-- Description: Updates reallocate_cancelled_delivery_fees to accurately refund 
-- multi-shop surcharge and heavy order fees to the customer, while preventing
-- double-counting these fees in rider earnings.
...
BEGIN
    ...
    -- 100x FIX: DO NOT REALLOCATE MULTI-SHOP SURCHARGE AND HEAVY ORDER FEE.
    ...
    LOOP
        ...
        SET delivery_charges = rec.delivery_charges + v_split_delivery,
            -- FIX: delivery_charges already includes multi_shop_surcharge and heavy_order_fee.
