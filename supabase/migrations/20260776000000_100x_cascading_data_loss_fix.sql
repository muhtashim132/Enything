-- =============================================================================
-- Migration: 100x Cascading Data Loss & Systemic Penalty Bleed Fix
-- Description:
--   1. Fixes the "Cascading Data Loss" in `place_orders_transaction`. The prior 
--      implementation securely rebuilt the order JSON to prevent injection but 
--      accidentally dropped critical operational fields (`customer_phone`, 
--      `shop_phone`, `shop_prep_time_snapshot`, `prescription_urls`, 
--      `gst_rate_snapshot`, etc).
--   2. Fixes the "Systemic Seller Penalty Bleed" where `shop_prep_time_snapshot`
--      defaulted to 0 due to being dropped, explicitly stealing money from sellers 
--      for their standard prep time!
-- =============================================================================
...
BEGIN
  ...
  FOR v_order_record IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    -- 100x FIX 1: Explicitly preserve all required operational fields that were previously dropped!
    -- This fixes the Systemic Seller Penalty Bleed (shop_prep_time_snapshot was null) and Comms Failure (phones were null)
    v_secure_order := v_secure_order || jsonb_build_object(
      ...
      'customer_phone', v_secure_order->>'customer_phone',
      'shop_phone', v_secure_order->>'shop_phone',
      'delivery_instructions', COALESCE(v_secure_order->>'delivery_notes', v_secure_order->>'delivery_instructions'),
      'shop_prep_time_snapshot', (v_secure_order->>'shop_prep_time_snapshot')::int,
      'prescription_urls', v_secure_order->'prescription_urls',
      'gst_rate_snapshot', (v_secure_order->>'gst_rate_snapshot')::numeric,
      'razorpay_payment_id', v_secure_order->>'razorpay_payment_id',
      'razorpay_order_id', v_secure_order->>'razorpay_order_id',
