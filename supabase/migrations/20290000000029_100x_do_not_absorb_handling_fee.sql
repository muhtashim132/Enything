-- =============================================================================
-- Migration: 100x Do Not Absorb Handling Fee
-- Description:
--   Fixes a major math bug in `reallocate_cancelled_delivery_fees` where 
--   active orders were forced to absorb the cancelled order's `platform_fee`,
--   `small_cart_fee`, and `heavy_order_fee`.
--   Platform fee is strictly a per-shop fee. If a shop cancels, its platform
--   fee should be completely refunded to the customer, NOT passed on to the 
--   remaining active shops.
-- =============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- We only extract the delivery pool because delivery is based on max distance of the whole cart
        SELECT 
        ...
        LOOP
            ...
            -- Absorb only the cart delivery fee to guarantee rider max-distance pay
            ...
            v_new_grand := GREATEST(0, rec.total_amount + rec.gst_item_total + COALESCE(rec.platform_fee, 0) + v_new_del + COALESCE(rec.small_cart_fee, 0) + COALESCE(rec.heavy_order_fee, 0) + v_new_surcharge - COALESCE(rec.coupon_discount, 0));
