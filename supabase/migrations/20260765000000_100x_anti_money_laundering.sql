-- =============================================================================
-- Migration: 100x Anti-Money Laundering Fix
-- Description:
--   1. Fixes the Delivery Fee spoofing exploit in place_orders_transaction. 
--      Previously, an attacker could inflate delivery_charges to launder money 
--      to a rider accomplice. Now, the backend recalculates the max allowed 
--      delivery fee based on estimated_distance_km and the platform config.
--   2. Fixes reallocate_cancelled_delivery_fees to include 'verification_failed' 
--      so riders don't lose delivery fees if a shop rejects an order for prescription reasons.
-- =============================================================================

-- 1. Fix Anti-Money Laundering Delivery Fee Spoofing
...
BEGIN
  ...
  FOR v_order IN SELECT * FROM jsonb_to_recordset(p_orders) AS x(
    ...
    -- 100x BUG FIX: Force customer_id to auth.uid() to prevent Ghost Checkout IDOR
...
-- 2. Fix Rider Delivery Fee Reallocation Hole
...
BEGIN
    ...
    -- 100x FIX: Added 'verification_failed' and 'shop_dispute' to ensure missing delivery fees
    -- from ALL rejected states are correctly reallocated to the rider!
    SELECT 
    ...
    FROM orders
    WHERE cart_group_id = p_cart_group_id
      AND status IN ('cancelled', 'seller_rejected', 'verification_failed', 'shop_dispute_cancel', 'shop_dispute')
    ...
    LOOP
        ...
        SET 
            ...
            delivery_discount = 0,
            gst_delivery = 0,
            grand_total_collected = GREATEST(0, (total_amount + gst_item_total + platform_fee + gst_platform) - coupon_discount)
        WHERE id = rec.id;
    END LOOP;

    -- Distribute to remaining active orders
    FOR rec IN 
        SELECT id, delivery_charges, rider_earnings, multi_shop_surcharge, small_cart_fee, heavy_order_fee, delivery_discount,
               total_amount, gst_item_total, platform_fee, gst_platform, coupon_discount 
        FROM orders 
        WHERE cart_group_id = p_cart_group_id 
          AND status = 'awaiting_payment'
    LOOP
        ...
        UPDATE orders
        SET 
            delivery_charges = v_net_delivery,
