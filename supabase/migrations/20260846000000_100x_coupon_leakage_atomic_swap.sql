-- =============================================================================
-- Migration: 100x Checkout Fee Sync Fix
-- Description:
--   1. Fixes platform_fee to be a flat cart fee (removing v_num_shops multiplier).
--   2. Syncs small_cart_threshold and heavy_order config fallbacks with frontend.
--   3. Relaxes dynamic multi-shop surcharge strict equality validation.
-- =============================================================================
...
DECLARE
  ...
  v_expected_multi_shop_surcharge NUMERIC := 0;
...
BEGIN
  ...
  IF jsonb_array_length(p_orders) > 3 THEN
    RAISE EXCEPTION 'Pixel Overload / DDoS Guard: Total shops per cart exceeds maximum platform limit (3).';
  END IF;

  SELECT SUM(quantity) INTO v_total_qty FROM jsonb_to_recordset(p_items) AS x(quantity int);
  IF v_total_qty IS NULL OR v_total_qty > 100 THEN
    RAISE EXCEPTION 'Pixel Overload / DDoS Guard: Total item quantity exceeds maximum platform limit (100).';
  END IF;

  IF p_idempotency_key IS NOT NULL THEN
    PERFORM pg_advisory_xact_lock(hashtext('idemp_' || p_idempotency_key));
  ...
  IF v_cart_group_id IS NULL AND p_idempotency_key IS NOT NULL THEN
    BEGIN
      v_cart_group_id := p_idempotency_key::uuid;
    EXCEPTION WHEN OTHERS THEN
      v_cart_group_id := NULL;
    END;
  END IF;
  ...
  IF p_coupon_id IS NOT NULL THEN
    SELECT usage_limit, usage_count, discount_type, discount_value, max_discount, min_order_amount, valid_from, valid_until, applies_to, shop_id
    INTO v_coupon_max_uses, v_coupon_current_uses, v_coupon_type, v_coupon_value, v_coupon_max_discount, v_coupon_min_order, v_coupon_valid_from, v_coupon_valid_until, v_coupon_applies_to, v_coupon_shop_id
    FROM coupons
    ...
    IF v_coupon_applies_to = 'first_order' THEN
      SELECT COUNT(*) INTO v_cust_order_count FROM orders WHERE customer_id = auth.uid() AND status != 'cancelled' AND status != 'seller_rejected';
      IF v_cust_order_count > 0 THEN
        RAISE EXCEPTION 'This coupon is only valid for your first order.';
      END IF;
    ELSIF v_coupon_applies_to = 'specific_shop' THEN
      FOR v_order_record IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
        IF (v_order_record.value->>'shop_id')::uuid != v_coupon_shop_id THEN
          RAISE EXCEPTION 'This coupon is only valid for a specific shop. You cannot use it on this cart.';
        END IF;
      END LOOP;
    END IF;
  END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(product_id uuid, variant_name text, price numeric, quantity int) LOOP
    IF v_item.quantity IS NULL OR v_item.quantity <= 0 THEN
      RAISE EXCEPTION 'Invalid negative, zero, or null quantity % for product %', v_item.quantity, v_item.product_id;
    END IF;
    ...
    IF v_db_price IS NULL THEN
      RAISE EXCEPTION 'Invalid product or variant: % / %. Product/variant does not exist.', v_item.product_id, COALESCE(v_item.variant_name, 'base');
    END IF;

    IF v_item.price IS NULL THEN
      RAISE EXCEPTION 'Item price cannot be null (Product %).', v_item.product_id;
    END IF;
  ...
  IF v_sum_expected_total_amount > 0 AND v_sum_expected_total_amount < v_global_small_cart_threshold THEN
    ...
    END IF;
  ELSE
    IF ABS(v_sum_client_small_cart_fee) > 1.0 THEN
  ...
  v_global_heavy_order_fee := COALESCE(v_global_heavy_order_fee, 20.0) * GREATEST(0, CEIL(v_total_weight_kg - v_global_heavy_order_threshold));
  ...
  IF v_total_weight_kg > v_global_heavy_order_threshold THEN
    ...
    END IF;
  ELSE
    IF ABS(v_sum_client_heavy_order_fee) > 1.0 THEN
  ...
  BEGIN SELECT value::numeric INTO v_delivery_rate_per_km FROM platform_config WHERE key = 'delivery_rate_per_km'; EXCEPTION WHEN OTHERS THEN v_delivery_rate_per_km := 10.0; END;
  v_delivery_rate_per_km := COALESCE(v_delivery_rate_per_km, 10.0);

  -- Multi-shop surcharge is dynamic based on distance on the client. 
  -- We ensure it's not negative, and if single shop, it's 0.
  IF v_num_shops > 1 THEN
    IF v_sum_client_multi_shop_surcharge < 0.0 THEN
      RAISE EXCEPTION 'Multi-shop surcharge spoofing detected. Cannot be negative.';
    END IF;
    -- AML CAP: Absolute max limit to prevent money laundering
    IF v_sum_client_multi_shop_surcharge > (v_num_shops * 100.0 * v_delivery_rate_per_km) THEN
      RAISE EXCEPTION 'AML Alert: Multi-shop surcharge exceeds maximum limits.';
    END IF;
  ELSE
    IF ABS(v_sum_client_multi_shop_surcharge) > 1.0 THEN
      RAISE EXCEPTION 'Multi-shop surcharge charged on single shop order.';
    END IF;
  END IF;

  SELECT MAX(COALESCE((x.order_data->>'estimated_distance_km')::numeric, 0)) INTO v_cart_distance_km
  FROM jsonb_array_elements(p_orders) AS x(order_data);

  -- 100x FIX: Expected delivery fee should EXACTLY match client calculation: 
  -- (Base + MultiShop + Heavy + Small) * (1 + GST)
  v_expected_delivery_fee := GREATEST(1, CEIL(v_cart_distance_km)) * v_delivery_rate_per_km;
  
  -- Add other surcharges (since they are bundled into delivery_charges by the client)
  v_expected_delivery_fee := (v_expected_delivery_fee + v_sum_client_multi_shop_surcharge + v_sum_client_heavy_order_fee + v_sum_client_small_cart_fee) * (1 + v_global_delivery_gst_rate);
  
  IF ABS(v_sum_client_delivery_charges - v_expected_delivery_fee) > 1.0 THEN
  ...
  FOR v_order_record IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    -- 100x FIX: Grand total must match OrderTaxBreakdown exact math.
    -- delivery_charges ALREADY includes multi_shop_surcharge, small_cart_fee, heavy_order_fee, AND GST.
    -- platform_fee ALREADY includes GST.
    -- Therefore, we ONLY add delivery_charges and platform_fee to the gross item total.
    -- DO NOT add small_cart_fee etc. again, or it will double count!
    ...
    -- Note: We extract GST from delivery_charges, but remember delivery_charges has ALL surcharges bundled.
    -- That's mathematically sound, because 18% GST applies to all of them.
    ...
    -- Rider earnings:
    -- In checkout_page.dart, riderEarnings is passed from the client, based on effectiveBase + surcharges.
    -- To ensure backend calculation matches exactly, we trust the client's riderEarnings since it's already verified 
    -- as a fixed proportion of the validated delivery_charges. But for total security, we calculate it dynamically:
    -- riderEarnings = (delivery_charges_without_gst - small_cart_fee) * riderPayoutRatio
    ...
    INSERT INTO orders (
      ...
      estimated_distance_km, cancellation_reason,
...
GRANT EXECUTE ON FUNCTION place_orders_transaction(JSONB, JSONB, UUID, UUID, TEXT, UUID) TO authenticated;

-- Legacy Timer Fix
CREATE OR REPLACE FUNCTION restart_payment_timer_single(p_order_id UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Bump the payment deadline and acceptance deadline for a single order
  UPDATE orders
  SET 
...
GRANT EXECUTE ON FUNCTION restart_payment_timer_single(UUID) TO authenticated;

-- Legacy Timer Fix for Group Orders (IDOR Patch)
