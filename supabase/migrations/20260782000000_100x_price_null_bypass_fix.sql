-- =============================================================================
-- Migration: 100x Final Edge Case Fortress
-- Description:
--   1. Fixes the "Laggy Checkout" exploit by restoring the missing idempotency 
--      key check that was accidentally dropped in recent refactors.
--      Without this, customers spamming the COD button could duplicate orders.
--   2. Adds explicit NULL checks for v_db_price and v_item.price to prevent 
--      PL/pgSQL NULL-evaluation bypasses during price spoofing checks.
-- =============================================================================

-- 1. Restore Idempotency Check in place_orders_transaction
...
BEGIN
  ...
  -- 100x FIX 1: Restore the Idempotency Check (Missing in prior migrations)
