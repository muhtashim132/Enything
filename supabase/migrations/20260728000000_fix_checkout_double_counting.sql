-- =============================================================================
-- Migration: Fix Checkout Double Counting of Surcharges
-- Description: Corrects the expected_grand_total calculation in the 
-- place_orders_transaction function to prevent double-counting of surcharges 
-- that are already included in delivery_charges and platform_fee.
-- =============================================================================
...
BEGIN
  ...
  FOR v_order IN SELECT * FROM jsonb_to_recordset(p_orders) AS x(
    ...
    -- delivery_charges already includes multi_shop_surcharge, small_cart_fee, heavy_order_fee, 
    -- gst_delivery, and subtracts delivery_discount.
