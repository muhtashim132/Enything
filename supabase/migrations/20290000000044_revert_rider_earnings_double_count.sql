-- =============================================================================
-- Migration: 20290000000044_revert_rider_earnings_double_count.sql
-- =============================================================================
-- HOTFIX: Reverts the rider_earnings formula in reallocate_cancelled_delivery_fees
-- and rebalance_active_delivery_fees back to the original correct formula.
--
-- Migration 043 incorrectly added v_new_surcharge to the rider base, but
-- surcharge is ALREADY embedded in v_new_del (the delivery pool retains allowed
-- surcharge; only the cancelled/ghost surcharge is removed). Adding it again
-- caused double-counting.
--
-- Correct formula: (delivery - gst - small) × rider_rate
--   which equals: (base + surcharge + heavy) × rider_rate
--   because delivery pool = (base + surcharge + heavy + small) × 1.18
-- =============================================================================
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            -- (only the ghost/cancelled surcharge was removed from the pool).
            -- So (v_new_del - gst - small) = base + surcharge + heavy,
            -- which matches checkout: riderBase = base + surcharge + heavy.
            -- DO NOT add v_new_surcharge — that would double-count.
            -- ───────────────────────────────────────────────────────────────
