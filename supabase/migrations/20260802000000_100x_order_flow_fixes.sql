-- =============================================================================
-- Migration: 100x Order Flow Fixes
-- Description:
--   1. Fixes set_arrived_at_shop to accept preparing/ready_for_pickup.
--   2. Fixes update_order_status to allow preparing from confirmed state.
--   3. Fixes delivered geo-fence check to use delivery_lat/lng directly from orders.
-- =============================================================================

-- 1. Fix set_arrived_at_shop
...
BEGIN
  ...
  IF p_new_status IN ('preparing', 'ready_for_pickup') THEN
    ...
    -- 100x FIX: Allow preparing from confirmed
  ...
  ELSIF p_new_status IN ('picked_up', 'out_for_delivery', 'delivered') THEN
    ...
    IF p_new_status = 'delivered' THEN
        ...
        -- 100x FIX: Enforce 300m Geo-Fence for Customer Location directly from orders
