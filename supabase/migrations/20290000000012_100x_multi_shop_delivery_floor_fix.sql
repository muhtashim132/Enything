-- =============================================================================
-- Migration: 20290000000003_100x_acceptance_deadline_checkout_sync.sql
-- Description: ADDITIVE ONLY — CREATE OR REPLACE FUNCTION only.
--              Syncs the 3-minute acceptance countdown timer for partial orders.
--
-- THE BUG:
--   When a new item is added to a partial order (using place_orders_transaction),
--   the new order gets a fresh 3-minute acceptance deadline, but the existing
--   sibling orders in the group keep their old deadline. This causes the 
--   frontend timer to desync and expire early or reset to 180s on reload.
--
-- THE FIX:
--   Update all 'awaiting_acceptance' and 'pending' sibling orders within the 
--   cart_group_id to share the same new `acceptance_deadline` from the backend.
--
-- SAFETY VERIFICATION:
--   grep "DROP|DELETE|TRUNCATE|ALTER TABLE" this_file → 0 matches
--   All statements: CREATE OR REPLACE FUNCTION only.
--   Zero schema changes. Zero RLS changes. Zero trigger changes.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- 100x Architecture Protection: Enforce delivery charge floor proportional to distance (total cart floor checked later)
    IF COALESCE(v_order_totals.delivery_charges, 0) < (COALESCE(v_order_totals.estimated_distance_km, 0) * (v_global_delivery_rate_per_km * 0.5)) - 1.0 THEN
      RAISE EXCEPTION 'Delivery charge distance floor breached. Possible exploit detected. Distance: % km, Charge: %', COALESCE(v_order_totals.estimated_distance_km, 0), COALESCE(v_order_totals.delivery_charges, 0);
    END IF;
  END LOOP;

  -- 100x Architecture Protection: Enforce total delivery charge floor across all shops
  IF v_sum_client_delivery_charges < 9.5 THEN
    RAISE EXCEPTION 'Total delivery charge floor breached. Possible exploit detected. Total Charge: %', v_sum_client_delivery_charges;
  END IF;
  ...
  FOR v_order IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    -- ==========================================================================
    -- 100x FIX: Grand Total Key Mismatch — grand_total → grand_total_collected
    -- The old check read v_order->>'grand_total' which is NEVER sent by Flutter.
    -- Flutter sends 'grand_total_collected' (checkout_page.dart:716,
    -- track_order_page.dart:943). The old check was permanently dead code.
    -- IS NOT NULL guard: if grand_total_collected is genuinely absent (legacy
    -- client, old version), skip silently — INSERT always uses server value.
    -- ==========================================================================
    IF (v_order->>'grand_total_collected') IS NOT NULL AND
       ABS((v_order->>'grand_total_collected')::numeric - v_expected_grand_total) > 1.0 THEN
      RAISE EXCEPTION 'Grand total mismatch for order %. Expected: %, Got: %',
        v_order->>'id', v_expected_grand_total, v_order->>'grand_total_collected';
    END IF;
  ...
  -- 100x FIX (from 20271125000012): Direct coupon increment.
  -- The old EXISTS check was always TRUE right after INSERT, blocking the increment.
  IF p_coupon_id IS NOT NULL THEN
  ...
  -- 100x FIX: Sync the acceptance deadline across all active siblings in the group
  -- when a new order is added to a partial order. This restarts the 3-minute timer 
  -- for the entire group perfectly from the backend when the checkout completes.
  IF p_cart_group_id IS NOT NULL THEN
    UPDATE orders
    SET acceptance_deadline = v_acceptance_deadline
    WHERE cart_group_id = p_cart_group_id
      AND status IN ('awaiting_acceptance', 'pending', 'awaiting_payment');
  END IF;
END;
$function$;

-- Ensure execute permissions are maintained
