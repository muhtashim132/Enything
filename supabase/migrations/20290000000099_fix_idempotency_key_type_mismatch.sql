-- =============================================================================
-- Migration: 20290000000093_fix_idempotency_key_type_mismatch.sql
-- =============================================================================
-- FIXES (ADDITIVE ONLY — no business logic changes):
--
--   1. Fixes PostgREST "operator does not exist: text = uuid (code: 42883)":
--      In place_orders_transaction(), orders.idempotency_key is of type TEXT in
--      PostgreSQL, but the RPC was casting `p_idempotency_key::uuid` in the
--      idempotency check and the INSERT statement.
--      Fix: Perform polymorphic safe comparison `idempotency_key::text = p_idempotency_key::text`
--      and insert `p_idempotency_key::text` directly.
--
--   2. Explicitly ensures orders.idempotency_key column is TEXT in public schema.
--
--   3. Maintains 100% of all migration 92 unification logic:
--      (Dynamic admin commission key aliasing, soft deletes guard,
--       PostGIS ST_Y/ST_X geospatial bounds, WeightEngine resolution,
--       reallocate_cancelled_delivery_fees, rebalance_active_delivery_fees).
-- =============================================================================

-- Ensure idempotency_key column in orders is explicitly TEXT
DO $$
BEGIN
  IF EXISTS (
...
-- ─────────────────────────────────────────────────────────────────────────────
-- FIX 1: place_orders_transaction — idempotency_key type alignment
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  -- Idempotency Check: if this key already exists, return the existing order details
  -- ADDITIVE FIX: Polymorphic safe text comparison prevents "operator does not exist: text = uuid"
