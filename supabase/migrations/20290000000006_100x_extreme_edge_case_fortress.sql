-- =============================================================================
-- Migration: 100x Extreme Edge Case Fortress
-- Description: Hardens the notification logic against cascading logic failures 
--              discovered during deep stress testing.
--              1. Replaces brittle NOT IN list with robust IN ('pending', 'awaiting_acceptance')
--              2. Replaces seconds epoch with milliseconds epoch for perfect transaction deduplication
-- =============================================================================

-- 1. Targeted Broadcast for Replacement Orders (Millisecond Precision)
...
BEGIN
  ...
  THEN
    ...
    IF v_existing_rider IS NOT NULL THEN
      ...
      -- 100x FIX: Millisecond precision to prevent any edge-case timestamp collisions
      v_notif_key := COALESCE(NEW.cart_group_id, NEW.id)::text || '_replacement_' || (extract(epoch from NEW.created_at) * 1000)::bigint::text;
    ...
    LOOP
      IF TG_OP = 'INSERT' THEN
        -- 100x FIX: Millisecond precision for exact transaction deduplication
        v_notif_key := COALESCE(NEW.cart_group_id, NEW.id)::text || '_new_available_' || (extract(epoch from NEW.created_at) * 1000)::bigint::text;
      ELSE
        v_notif_key := COALESCE(NEW.cart_group_id, NEW.id)::text || '_reassigned_' || (extract(epoch from now()) * 1000)::bigint::text;
      END IF;
...
BEGIN
  ...
  -- 100x FIX: Robust inclusion check. We ONLY block payment notifications if other shops are strictly 'pending' or 'awaiting_acceptance'.
  -- This is vastly superior to the brittle NOT IN list which could fail if new statuses are added or legacy ones hit.
  ...
  IF TG_OP = 'UPDATE' AND (NEW.status != OLD.status OR NEW.seller_accepted != OLD.seller_accepted) THEN
    ...
    ELSIF NEW.status != OLD.status THEN
      ...
      IF v_title IS NOT NULL THEN
        -- Include updated_at epoch to deduplicate simultaneous transitions perfectly (millisecond precision)
        IF NEW.status IN ('awaiting_payment', 'partner_rejected', 'cancelled') THEN
          v_notif_key := COALESCE(NEW.cart_group_id, NEW.id)::text || '_' || NEW.status || '_' || (extract(epoch from NEW.updated_at) * 1000)::bigint::text;
        ELSE
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    INSERT INTO public.notifications (user_id, notif_key, title, body, order_id)
    VALUES (NEW.customer_id, COALESCE(NEW.cart_group_id, NEW.id)::text || '_placed_' || (extract(epoch from NEW.created_at) * 1000)::bigint::text, '🛍️ Order Sent!', 'Waiting for the shop & rider to accept. No charge yet — you pay only after both confirm.', NEW.id)
    ON CONFLICT (user_id, notif_key) DO NOTHING;
