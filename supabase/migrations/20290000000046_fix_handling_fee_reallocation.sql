-- 20290000000046_fix_handling_fee_reallocation.sql
-- BUG FIX: reallocate_cancelled_delivery_fees was stealing the platform_fee
-- from cancelled shops and forcing the remaining active shops to absorb it.
-- This caused the "Handling Fee" to artificially inflate when a shop was rejected.
-- FIX: Platform fee is a per-shop fee. Active shops should simply retain their 
-- original platform_fee, and the cancelled shop's platform_fee should be left in the refund pool.
...
BEGIN
    ...
    LOOP
        ...
        -- 100x FIX: We explicitly DO NOT sum `platform_fee` into a pool. 
        -- It is a per-shop fee, so the cancelled shop's platform_fee should NOT be aggregated 
        -- and forced onto the remaining active shops.
        SELECT 
        ...
        FOR rec IN
            SELECT id, total_amount, gst_item_total, coupon_discount, platform_fee
              FROM orders
             WHERE cart_group_id = p_cart_group_id
               AND razorpay_payment_id IS NOT DISTINCT FROM pay_rec.razorpay_payment_id
               AND status NOT IN ('cancelled', 'seller_rejected', 'timeout', 'payment_failed', 'shop_dispute_cancel')
        LOOP
            ...
            -- 100x FIX: Active shops simply retain their exact per-shop platform fee!
            v_new_plat      := rec.platform_fee;
