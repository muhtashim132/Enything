-- =============================================================================
-- Migration: 100x Secure Fees, Coupons Math, and GST 2.0
-- Description:
--   1. Enforces small_cart_fee and heavy_order_fee strictly based on total amount and weight.
--   2. Enforces multi_shop_surcharge if order has multiple shops.
--   3. Re-implements GST 2.0 overrides and price slabs for Clothing/Footwear.
--   4. Validates coupon discount mathematically against the coupon definition.
-- =============================================================================
...
BEGIN
  ...
  -- 1. Validate Base Prices (variant-aware) and Calculate Total Weight/Amount
  ...
  -- 100x SECURITY FIX: STRICT ENFORCEMENT OF SMALL CART FEE
  ...
  -- 100x SECURITY FIX: STRICT ENFORCEMENT OF HEAVY ORDER FEE
  ...
  -- 100x SECURITY FIX: STRICT ENFORCEMENT OF MULTI-SHOP SURCHARGE
  ...
  FOR v_order IN SELECT * FROM jsonb_to_recordset(p_orders) AS x(
    ...
    LOOP
      ...
      -- 100x SECURITY FIX: GST 2.0 OVERRIDES AND SLABS
