-- =============================================================================
-- Migration: 20290000000091_fix_gateway_deduction_gst_rate.sql
-- =============================================================================
-- Description:
--   ADDITIVE FIX: The gateway deduction formula in place_orders_transaction()
--   was incorrectly using v_platform_gst_rate (admin-configurable) for the
--   GST component of the Razorpay gateway fee. The gateway fee's GST is a
--   fixed 18% (Razorpay charges 2% + 18% GST = 2.36%), independent of the
--   platform fee's GST rate. If admin changed platform_fee_gst_rate, the
--   gateway deduction would diverge from the frontend's fixed 2.36%.
--
--   Fix: Use hardcoded 0.18 for gateway fee GST component.
--   Formula change: (grand * 0.02) * (1.0 + v_platform_gst_rate)
--                 → (grand * 0.02) * (1.0 + 0.18)
--                 = grand * 0.0236 (matching frontend TaxConfig)
-- =============================================================================
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(
    ...
    IF v_unit_weight IS NOT NULL AND v_unit_weight > 0 THEN
      v_clean_unit := LOWER(TRIM(COALESCE(v_unit_type, '')));
      IF v_clean_unit IN ('grams', 'gram', 'g', 'gm', 'ml', 'milliliter', 'milliliters') THEN
  ...
  IF v_total_weight_kg > v_global_heavy_order_threshold THEN
    ...
    IF NOT v_is_replacement AND ABS(v_sum_client_heavy_order_fee - v_expected_heavy_order_fee) > 1.5 THEN
       -- Also allow flat heavy fee comparison
  ...
  FOR v_order IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    -- BUG FIX: Use fixed 18% GST for gateway fee (Razorpay's fee), NOT v_platform_gst_rate.
    -- Gateway fee = 2% of grand total, GST on gateway fee = 18% (always fixed by Razorpay).
    -- Total effective rate = 2% × 1.18 = 2.36%, matching frontend TaxConfig.effectiveGatewayDeductionPercent.
