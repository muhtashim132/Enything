-- =============================================================================
-- PILLAR 5: DELIVERY FEE DOUBLE-DIPPING & DEAD RIDER FIX
-- =============================================================================
-- Description: 
-- When a user replaces an order, Enything's reallocation logic combined with the 
-- client-side UI calculation causes a cascading delivery fee double-charge.
-- If we just zero the UI fee, the replacement rider gets 0 payout (Dead Rider).
-- This migration introduces `rebalance_active_delivery_fees` which takes the 
-- total delivery funds currently held by all active orders in the cart group and 
-- redistributes them evenly, ensuring Enything takes no extra cut and every rider gets paid.
...
BEGIN
  ...
  LOOP
    ...
    SET delivery_charges = v_split_delivery,
        ...
        -- Recalculate rider earnings dynamically (Formula: ((Base - GST) - SmallCart) * 0.8)
        rider_earnings = ((v_split_delivery - (v_split_delivery * 0.18)) - v_split_small) * 0.80,
...
-- Patch place_orders_transaction to trigger rebalance ONLY on atomic swap
