-- =============================================================================
-- Migration: 20290000000067_update_gst_tcs_rate_to_half_percent.sql
-- =============================================================================
-- Description:
--   1. Updates GST TCS rate (§52 CGST Act) from 1% to statutory 0.5%
--      (0.25% CGST + 0.25% SGST) in accordance with Notification No. 15/2024-Central Tax.
--   2. Dynamically reads slab_threshold and slab_high_rate from tax_config for
--      Clothing and Footwear categories.
-- =============================================================================

DROP FUNCTION IF EXISTS public.place_orders_transaction(jsonb, jsonb, uuid, uuid, text, uuid);
...
BEGIN
  ...
  FOR v_order IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    SELECT lat, lng INTO v_shop_lat, v_shop_lng FROM shops WHERE id = (v_order->>'shop_id')::uuid;
  ...
  FOR v_order IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(product_id uuid, variant_name text, price numeric, quantity int, order_id uuid) WHERE order_id = (v_order->>'id')::uuid LOOP
      ...
      -- GST TCS (§52 CGST Act & Notification 15/2024-CT: 0.5% on non-food taxable goods, 0% on §9(5) and fresh produce)
  ...
  LOOP
    INSERT INTO order_items (id, order_id, product_id, quantity, price, variant_name)
    VALUES (
  ...
  IF p_coupon_id IS NOT NULL THEN
    ...
    IF v_coupon_record.valid_from IS NOT NULL AND now() < v_coupon_record.valid_from THEN
      RAISE EXCEPTION 'Coupon is not yet active';
    END IF;

    IF v_coupon_record.valid_until IS NOT NULL AND now() > v_coupon_record.valid_until THEN
    ...
    IF v_coupon_record.usage_limit IS NOT NULL THEN
      ...
      IF v_total_usage_count >= v_coupon_record.usage_limit THEN
        RAISE EXCEPTION 'Coupon usage limit reached';
      END IF;
    END IF;

    IF v_coupon_record.per_user_limit IS NOT NULL THEN
    ...
    IF v_coupon_record.min_order_amount IS NOT NULL THEN
      IF v_sum_expected_total_amount < v_coupon_record.min_order_amount THEN
        RAISE EXCEPTION 'Cart total does not meet the minimum order amount of % for this coupon', v_coupon_record.min_order_amount;
      END IF;
    END IF;
    ...
    IF v_coupon_record.discount_type = 'percentage' THEN
      ...
      END IF;
    ELSE
      v_calculated_discount := LEAST(v_coupon_record.discount_value, v_sum_expected_total_amount);
    END IF;

    v_calculated_discount := GREATEST(0, v_calculated_discount);

    SELECT COALESCE(SUM((x->>'coupon_discount')::numeric), 0)
    INTO v_client_claimed_discount
    FROM jsonb_array_elements(p_orders) AS x;

    IF ABS(v_client_claimed_discount - v_calculated_discount) > 1.0 THEN
      RAISE EXCEPTION 'Coupon discount mismatch. Calculated: %, Claimed: %', v_calculated_discount, v_client_claimed_discount;
    END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'item_ids', to_jsonb(v_inserted_item_ids),
