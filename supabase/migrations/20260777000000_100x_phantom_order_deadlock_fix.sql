-- =============================================================================
-- Migration: 100x Phantom Order Deadlock Fix
-- Description:
--   Fixes the Phantom Order Deadlock in `place_orders_transaction`.
--   The transaction failed to set `acceptance_deadline` during order creation,
--   leaving it `NULL`. This caused the auto-cancel cron jobs to completely ignore
--   unresponsive orders, locking the customer's intent indefinitely.
--   This migration proactively injects `acceptance_deadline = NOW() + 2 minutes`.
-- =============================================================================
...
BEGIN
  ...
  FOR v_order_record IN SELECT * FROM jsonb_array_elements(p_orders) LOOP
    ...
    -- 100x FIX 3: Inject acceptance_deadline natively!
