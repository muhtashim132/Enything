-- =============================================================================
-- Migration: 100x Rejection Stress Test Fortress
-- Description:
--   1. Fixes an Inventory Deadlock Vulnerability where `IN (SELECT...)` failed 
--      to guarantee PostgreSQL lock ordering on the `products` table.
--   2. Fixes a Pixel Overload / OOM Blob attack vulnerability by capping the
--      rejection message at 500 characters to prevent massive JSON payloads.
-- =============================================================================
...
DECLARE
  ...
  v_product_id uuid; -- Added for deadlock-free looping
BEGIN
  ...
  -- STRESS TEST FIX: Prevent Deadlocks by ordering the product updates 
  IF p_reject_reason = 'out_of_stock' THEN
    -- Using a deterministic lock order to prevent deadlock with trg_restore_stock_on_cancel_stmt
    FOR v_product_id IN 
      SELECT product_id FROM order_items WHERE order_id = p_order_id ORDER BY product_id 
    LOOP
      UPDATE products
  ...
  SET 
    ...
    -- STRESS TEST FIX: Prevent Pixel Overload / OOM from massive text blobs
