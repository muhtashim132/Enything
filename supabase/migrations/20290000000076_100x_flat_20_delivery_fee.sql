-- =============================================================================
-- Migration: 20290000000076_100x_flat_20_delivery_fee.sql
-- =============================================================================
-- Description:
--   1. Seeds delivery_base_fee = '20.0' and multi_shop_surcharge = '0.0' in platform_config.
--   2. Updates place_orders_transaction to enforce flat delivery fee per cart
--      (1, 2, or 3 shops covered by the single flat delivery fee).
--   3. Seamlessly handles replacement/additive orders with 0 additional delivery fee.
-- =============================================================================

INSERT INTO public.platform_config (key, value)
VALUES 
  ('delivery_base_fee', '20.0'::jsonb),
  ('multi_shop_surcharge', '0.0'::jsonb)
...
DECLARE
  ...
  v_global_multi_shop_surcharge numeric := 0.0;
...
BEGIN
  ...
  -- Validate Flat Delivery Fee (covers 1, 2, or 3 shops in cart)
  IF p_order_id_to_cancel IS NOT NULL THEN
    v_expected_delivery_fee := 0.0;
  ELSE
    v_expected_delivery_fee := (v_global_delivery_base + v_sum_client_multi_shop_surcharge + v_sum_client_heavy_order_fee) * (1.0 + v_delivery_gst_rate);
  END IF;
