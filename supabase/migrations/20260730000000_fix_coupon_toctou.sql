-- =============================================================================
-- Migration: Fix Coupon TOCTOU Exploit
-- Description:
--   1. Adds FOR UPDATE lock to the coupon usage check in place_orders_transaction
--      to prevent concurrent checkout requests from bypassing the usage limit.
-- =============================================================================
...
BEGIN
  ...
  IF p_coupon_id IS NOT NULL THEN
    ...
    FROM coupons
    WHERE id = p_coupon_id FOR UPDATE; -- <--- FIX: Added FOR UPDATE to prevent TOCTOU
