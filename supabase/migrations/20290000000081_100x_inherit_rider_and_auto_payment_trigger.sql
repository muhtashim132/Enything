-- =============================================================================
-- Migration: 20290000000081_100x_inherit_rider_and_auto_payment_trigger.sql
-- =============================================================================
-- Description:
--   1. Automatically inherits delivery_partner_id, partner_accepted, and rider_phone
--      when an additive/replacement order is placed into an existing cart_group_id
--      where a rider was already assigned.
--   2. Ensures the replacement order immediately advances to 'awaiting_payment'
--      as soon as the shop owner accepts via accept_order_seller.
-- =============================================================================
...
BEGIN
  ...
  -- 100x FIX: Inherit assigned rider from existing active orders in this cart group
  ...
  IF v_total_weight_kg > v_global_heavy_order_threshold THEN
    IF NOT v_is_replacement AND ABS(v_sum_client_heavy_order_fee - v_global_heavy_order_fee) > 1.0 THEN
