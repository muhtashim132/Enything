-- =============================================================================
-- Migration: 100x Seller Dashboard Fixes
-- Description:
--   1. Fixes get_seller_balance to accurately deduct refunded/disputed orders.
--   2. Fixes get_seller_daily_stats to deduct refunded/disputed orders from today's earnings.
--   3. Secures place_orders_transaction to abort if a product is hidden (is_available=false) or deleted (is_deleted=true).
-- =============================================================================

-- 1. Fix get_seller_balance
...
BEGIN
  ...
  -- FIX: Deduct refunded or processing refunds from the balance accurately
...
BEGIN
    ...
    -- FIX: Deduct refunded or processing refunds from today's earnings
    ...
    -- Get products count
    -- Also only count active products to be more accurate (optional enhancement, but keeping it as all non-deleted for now)
...
-- 3. Fix place_orders_transaction (Security Vulnerability Patch)
...
BEGIN
  ...
  -- 1. Validate Base Prices AND Availability (Security Fix)
  FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(product_id uuid, variant_name text, price numeric) LOOP
    ...
    -- FIX: Abort if product is hidden or deleted (Security bypass fix)
