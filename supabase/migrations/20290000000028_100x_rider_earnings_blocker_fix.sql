-- =============================================================================
-- Migration: 100x Rider Earnings Blocker Fix
-- Description:
--   Removes the `AND COALESCE(rider_earnings, 0) = 0` condition from 
--   `reallocate_cancelled_delivery_fees`. Since `rider_earnings` is populated 
--   with a projected non-zero value at checkout, this condition was silently 
--   aborting the reallocation logic for EVERY single cancellation.
-- =============================================================================
...
BEGIN
    ...
    FOR pay_rec IN
        ...
        WHERE cart_group_id = p_cart_group_id
          ...
          -- 100x BUG FIX: Removed `AND COALESCE(rider_earnings, 0) = 0` which blocked the trigger from running!
    LOOP
