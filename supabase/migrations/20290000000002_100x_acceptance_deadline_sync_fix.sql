-- =============================================================================
-- Migration: 20290000000002_100x_acceptance_deadline_sync_fix.sql
-- Description: ADDITIVE ONLY — CREATE OR REPLACE FUNCTION only.
--              Fixes the 3-minute acceptance countdown timer reset bug for online payments.
--
-- THE BUG:
--   v_acceptance_deadline was only set for COD orders. For online payments, it was NULL.
--   Because the backend doesn't provide a deadline, the frontend (track_order_page.dart) 
--   falls back to a hardcoded 180 seconds every time the page is opened, 
--   causing the timer to constantly reset.
--
-- THE FIX:
--   Change to: v_acceptance_deadline := (now() + interval '3 minutes');
--   This ensures both COD and online orders have a strict server-enforced 
--   acceptance deadline that the frontend can accurately sync with.
--
-- SAFETY VERIFICATION:
