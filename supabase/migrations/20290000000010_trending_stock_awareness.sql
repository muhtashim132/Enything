-- ============================================================================
-- Migration: 20290000000010_trending_stock_awareness.sql
-- Description: ADDITIVE ONLY — Injects stock awareness into Trending logic.
--              Filters out products that are physically out of stock
--              (total_quantity <= 0) to prevent UX frustration where users
--              see trending products they cannot purchase.
-- ============================================================================
...
AS $$
  ...
  FROM (
    ...
    WHERE p.is_available = true
      -- INVENTORY CHECK: Product must have stock (or unmanaged infinite stock)
...
AS $$
  ...
  FROM (
    ...
    WHERE p.is_available = true
      ...
      -- Strictly respect admin's provided radius
      AND ST_DWithin(
             s.location,
