...
Future<void> runShopEdgeCaseTests(SupabaseClient client) async {
  ...
  final fetchO1 = await client
      ...
      .eq('id', o1)
      .single();
  if (fetchO1['status'] != 'awaiting_acceptance' &&
      fetchO1['status'] != 'pending') {
    throw Exception(
        'Test 1 Failed: Status should be awaiting_acceptance, got ${fetchO1['status']}');
  }
  ...
  if (fetchO2['status'] != 'awaiting_payment') {
    throw Exception(
        'Test 2 Failed: Status should be awaiting_payment, got ${fetchO2['status']}');
  }
  ...
  await client.auth.signInWithPassword(
      email: _emailFromPhone(otherSellerPhone),
      password: _passwordFromPhone(otherSellerPhone));
  try {
    await client.rpc('accept_order_seller', params: {'p_order_id': o3});
    throw Exception(
        'Test 3 Failed: Unauthorized seller succeeded in accepting the order');
  } catch (e) {
    if (!e.toString().contains('Unauthorized')) {
      throw Exception(
          'Test 3 Failed: Unexpected error for unauthorized caller: $e');
    }
  ...
  print(
      'res4 exitCode: ${res4.exitCode}, stdout: ${res4.stdout}, stderr: ${res4.stderr}');
  await client.auth.signInWithPassword(
      email: _emailFromPhone(sellerPhone),
      password: _passwordFromPhone(sellerPhone));
  try {
    ...
    if (!e.toString().contains('ORDER_CANCELLED')) {
      throw Exception(
          'Test 4 Failed: Unexpected error for cancelled order: $e');
    }
  ...
  try {
    ...
    if (!e.toString().contains('Order not found')) {
      throw Exception(
          'Test 6 Failed: Unexpected error for order not found: $e');
    }
