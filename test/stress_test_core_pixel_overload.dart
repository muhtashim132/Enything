...
Future<void> main() async {
  ...
  Future<void> testConstraint(
      String scenario, String query, String expectedError) async {
    print('\n[Testing] $scenario');
    ...
    try {
      final res = await Process.run(
          'supabase', ['db', 'query', '-f', 'temp_query.sql', '--linked']);
      if (res.exitCode == 0) {
        print('❌ FAILED: Query succeeded but it should have been blocked!');
        failed++;
      } else {
        if (res.stderr.toString().contains(expectedError) ||
            res.stdout.toString().contains(expectedError)) {
