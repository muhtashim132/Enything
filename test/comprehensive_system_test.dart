...
Future<void> runComprehensiveTests(SupabaseClient client) async {
  ...
  final s2Shop =
      await client.from('shops').select('id').eq('seller_id', s2Id).single();
  ...
  await client.auth.signInWithPassword(
      email: _emailFromPhone(s1Phone), password: _passwordFromPhone(s1Phone));
  ...
  await client.auth.signInWithPassword(
      email: _emailFromPhone(s2Phone), password: _passwordFromPhone(s2Phone));
  await client.from('products').insert([
    {
      'id': p2Id,
      'shop_id': s2Shop['id'],
  ...
  await client.auth.signInWithPassword(
      email: _emailFromPhone(d1Phone), password: _passwordFromPhone(d1Phone));

  bool o4Failed = false;
  await client.rpc('accept_order_rider', params: {
    'p_order_id': o1,
    'p_rider_phone': d1Phone,
    'p_shop_lat': 34.0,
    'p_shop_lng': 74.0
  });
  ...
  await client.auth.signInWithPassword(
      email: _emailFromPhone(d1Phone), password: _passwordFromPhone(d1Phone));
  await client.rpc('set_arrived_at_shop',
      params: {'p_order_id': o1, 'p_rider_lat': 34.0, 'p_rider_lng': 74.0});
  await client.rpc('update_order_status', params: {
    'p_order_id': o1,
...
Future<String> _placeOrder(SupabaseClient client, String phone,
    String customerId, String shopId, String productId, double price) async {
  await client.auth.signInWithPassword(
      email: _emailFromPhone(phone), password: _passwordFromPhone(phone));
