-- =============================================================================
-- Migration: 20290000000025_100x_partial_rejection_final_fix.sql
-- Description: ADDITIVE ONLY — CREATE OR REPLACE FUNCTION only.
--   Issue 3 Safety Net: place_orders_transaction now also bypasses the delivery
--   floor check when p_cart_group_id already has active orders for this customer.
--   This handles the "Search for Different Items" edge case where pendingOrderIdToCancel
--   was not set, causing the floor check to fire on a valid ''0''' replacement charge.
--   See: track_order_page.dart: setPendingOrderIdToCancel (primary Dart fix).
--   This SQL is the secondary safety net.
-- UNDOABLE: Re-deploy migration 20290000000023 to restore the original behaviour.
-- =============================================================================

-- =============================================================================
-- Migration: 20290000000023_100x_checkout_floor_fortification.sql
-- Description: ADDITIVE ONLY — CREATE OR REPLACE FUNCTION only.
--              1. Replaces flawed `v_existing_group_orders` with rigorous
--                 `p_order_id_to_cancel` verification to prevent free delivery exploit.
--              2. Fixes summation bug by moving the delivery floor check
--                 outside the individual shop loop to evaluate the whole cart.
--
-- SAFETY VERIFICATION:
...
BEGIN
  ...
  LOOP
    ...
    v_sum_estimated_distance_km := v_sum_estimated_distance_km + COALESCE(v_order_totals.estimated_distance_km, 0);
  ...
  IF NOT v_is_valid_replacement THEN
    IF v_sum_client_delivery_charges < GREATEST(10.0, v_sum_estimated_distance_km * (v_global_delivery_rate_per_km * 0.5)) THEN
      RAISE EXCEPTION 'Total delivery charge floor breached. Possible exploit detected. Distance: % km, Charge: %', v_sum_estimated_distance_km, v_sum_client_delivery_charges;
    END IF;
  END IF;
